function [R_LongQun,T_LongQun,R_LongQun_mp,T_LongQun_mp,Xc3]=LongQuan3Point(x3d_h,x2d_h_normlized)
%在这个方程中，首先利用LongQuan方法建立一元8次方程，并求解，然后再用第四个点来判断哪个点是真实的
...
solution_for_x1=zeros(1,4);%4个组合，123,124,134,234，每个组合可以求解出四个解，所以是4*4的一个组合，每行代表每个组合的四个解%总共四个组合
solution_for_x2=zeros(1,4);
solution_for_x3=zeros(1,4);
% solution_for_x4=zeros(1,4);
...
solution_group1=roots(coefficient_class8_group1);
solution_group1=real(solution_group1);
...
% solution_group2=roots(coefficient_class8_group2);
% solution_group2=real(solution_group2);
% 
...
% solution_group3=roots(coefficient_class8_group3);
% solution_group3=real(solution_group3);
% 
...
% solution_group4=roots(coefficient_class8_group4);
% solution_group4=real(solution_group4);

residual_total=zeros(1,4);
verify_error=100;
for solution_index=1:length(solution_group1)%group1,2,3
    if isreal(solution_group1(solution_index))
        if solution_group1(solution_index)>0
            ...
            possible_solution_for_x2=real(roots(coefficient_for_x2));
            ...
            possible_solution_for_x3=real(roots(coefficient_for_x3));
...
%            introduce 3P absolute orientation
            x3d_camera_p1=solution_for_x1(1,solution_index)*x2d_h_normlized(1,:)./norm(x2d_h_normlized(1,:));
            x3d_camera_p2=solution_for_x2(1,solution_index)*x2d_h_normlized(2,:)./norm(x2d_h_normlized(2,:));
            x3d_camera_p3=solution_for_x3(1,solution_index)*x2d_h_normlized(3,:)./norm(x2d_h_normlized(3,:));
...
R_LongQun_mp=R_LongQun;
T_LongQun_mp=T_LongQun;
% solution_for_x1=zeros(1,4);%4个组合，123,124,134,234，每个组合可以求解出四个解，所以是4*4的一个组合，每行代表每个组合的四个解%总共四个组合
% solution_for_x2=zeros(1,4);
% solution_for_x3=zeros(1,4);
% solution_for_x4=zeros(1,4);
% [aa8,aa6,aa4,aa2,aa0]=get_forth_time_parameter_zhangdata(mp(x3d_h(1:3,1)),mp(x3d_h(1:3,2)),mp(x3d_h(1:3,3)),mp(x2d_h_normlized(1:3,1)),mp(x2d_h_normlized(1:3,2)));
% coefficient_class8_group1=[aa8,aa6,aa4,aa2,aa0];
% solution_group1=roots(coefficient_class8_group1);
% solution_group1=real(solution_group1);
% 
...
% residual_total=zeros(1,4);
% verify_error_mp=100;
% for solution_index=1:4%group1,2,3
%     if isreal(solution_group1(solution_index))
%         if solution_group1(solution_index)>0
%             solution_for_x1(1,solution_index)=sqrt(solution_group1(solution_index));
%             coefficient_for_x2=[1,-2*solution_for_x1(1,solution_index)*cos(theta12),solution_for_x1(1,solution_index)^2-d12^2];
...
%             p4_camera=Rp3*((x3d_h(4,1:3))'+tp3);
%             p4_error=norm([p4_camera(1)/p4_camera(3),p4_camera(2)/p4_camera(3)]-x2d_h_normlized(4,1:2));
%             if p4_error<verify_error_mp
%                 R_LongQun_mp=Rp3;
%                 T_LongQun_mp=Rp3*tp3;
%                 verify_error_mp=p4_error;
%             end
%             residual_total(1,solution_index)=residual2+residual3;
%         end
%     end
% end
% 
% for solution_index=1:4%group1,2,4
...
% end
% 
% for solution_index=1:4%group1,3,4
...
%     end
% end
% for solution_index=1:4%group2,3,4
...
% % rotation_matrix1=VR1*UR1';
% % rotation_matrix=rotation_matrix*[1,0,0;0,-1,0;0,0,1];
% tp4=1/scale_camera_world4*Rp3'*camera_coor_mean'-world_coor_mean';
% tic
% Rp3=1;
% tp3=1;
% Xc3=1;
