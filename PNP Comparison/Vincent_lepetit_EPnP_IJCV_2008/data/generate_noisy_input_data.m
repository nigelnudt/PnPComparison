...
i=1;
while i<=n
    xcam=random(xini,xend,1);
    ycam=random(yini,yend,1);
    point(i).Xcam(1,1)=xcam;
    point(i).Xcam(2,1)=ycam;
    point(i).Xcam(3,1)=random(zini,zend,1);
%     point(i).Xcam(3,1)=distance-(c*(-a*xcam-b*ycam))+0.1*(rand-0.5);%平面退化情形
