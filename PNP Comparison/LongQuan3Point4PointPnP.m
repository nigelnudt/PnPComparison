function [Rp3,tp3,Xc3,Rp4,tp4,Xc4]=LongQuan3Point4PointPnP(x3d_h,x2d_h_normlized)
%此方程主要比较了利用3组分别解方程求出X后平均的解与原文的解，以及5点平均的解在不同性能下的比较，并估算计算时间。
...
%三个未知数，基于Sylvester约减
% x3d_h=mp(x3d_h);
% x2d_h_normlized=mp(x2d_h_normlized);
num_point=4;
xworld=x3d_h(1:4,1);
yworld=x3d_h(1:4,2);
zworld=x3d_h(1:4,3);
...
solution_confirmed_x1=median([solution_for_x1(1,best_set_index(1)),solution_for_x1(2,best_set_index(2)),solution_for_x1(3,best_set_index(3))]);
solution_confirmed_x2=median([solution_for_x2(1,best_set_index(1)),solution_for_x2(2,best_set_index(2)),solution_for_x2(4,best_set_index(4))]);
solution_confirmed_x3=median([solution_for_x3(1,best_set_index(1)),solution_for_x3(3,best_set_index(3)),solution_for_x3(4,best_set_index(4))]);
solution_confirmed_x4=median([solution_for_x4(2,best_set_index(2)),solution_for_x4(3,best_set_index(3)),solution_for_x4(4,best_set_index(4))]);
